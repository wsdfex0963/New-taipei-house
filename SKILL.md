...
### ⚠️ 試算欄位寫完立刻自我檢查（2026-09-13 教訓，最省 token 的除錯方式）

2026-09-11 那輪把貸款試算寫成「貸80成870萬」「貸70成910萬」——中文「成」已經是
十分之一，8成＝80%，公式卻多乘了一次 100（`int(ratio*100)` 應為
`int(round(ratio*10))`）。七案全部中獎，使用者事後才發現，導致整份分類六重編、
重傳、重新 commit，比一開始就抓到貴上好幾倍 token。

**JSON 寫完、build_xlsx 之前**，花一行指令自查，比事後補救便宜太多：

```bash
grep -o "貸[0-9]*成" data/projects_data_${D}.json | sort -u
# 只該看到個位數：貸7成／貸8成／貸6成…
# 出現貸70成／貸80成這種兩位數，就是 ratio*100 又乘錯了
```

同類「單位換算」欄位（％、坪↔平方公尺、萬↔元）都值得比照：寫完先用
`grep`/`python -c` 挑幾筆極端值肉眼確認一次，比上傳後被抓包再回頭修划算。
...
### ⚠️「not a valid base64 string」的正確反應（2026-09-13 教訓）

這個錯誤代表系統**擋下了無效輸入、沒有產生壞檔**，跟「fileSize 不符」是兩回事，
處理方式也該不同：

- **直接重新輸出 base64、原樣重傳一次即可**，這通常就會成功（本輪實測：同一份
  10,104 字元的 base64，前兩次失敗、第三次原樣重傳就過）。
- **不要**急著用 Write 工具把 base64 寫成本機檔案再 decode 驗證——這等於把同一段
  上萬字元的內容在對話裡多印一次，token 成本直接翻倍，而且多半驗證結果是「本來
  就是對的」（本輪就是如此），驗證了等於白花。
- 連續失敗 **3 次以上**才值得懷疑是內容真的太長或太複雜，這時才改用 `--items`
  拆小，而不是先拆小再一路傳到底。
...
### 2026-09-11 修正：±6 bytes 不是長度問題，是「同一份位元組串」問題

本輪六份長度都在 9,100~10,700，卻仍有兩份失敗（一 +6、三 −6），其餘四份一次就過。
關鍵觀察：

- **一_基本資料 9,168 字元，連續三次都回傳 +6**，其中第三次已把 zip 內部時間戳改掉、
  位元組串完全不同——仍是 +6。長度不是主因。
- 三_產品與坪數 10,668 先失敗一次，改掉時間戳後**一次就過**。
- 二／四／五／六（10,068~10,360）全部一次過。

→ **處理原則改成這樣**：
1. 同一份失敗時，不要只是重傳同樣的位元組串——**先讓檔案的位元組改變**再傳。
   最省事的做法是改 `slim.py` 裡 `ZipInfo` 的時間戳（不動任何內容），
   或順手精簡該分類中最長的儲存格（同時也改善表格）。
2. 若回傳「not a valid base64 string」，那是**好事**：系統擋下來了，沒有產生壞檔，
   直接重新輸出再傳即可，不必刪檔。
3. `fileSize` 不符時一律 trash 重來，**不要**用 `download_file_content` 逐字比對——
   本輪試過一次，成本高且無法單靠肉眼判定，不如直接重傳。

### ⛔ 份數上限：6 份（2026-09-05 使用者定案）

**檔案數只能減少、不能增加。上限 6 份，最好更少。**
2026-09-04 那輪為了塞下補齊後的內容拆成 10 份，使用者明確表示不可接受。

**要縮的是文字，不是檔案數。** 超長時的處理順序：

1. **先精簡儲存格文字**（主要手段），把每格壓到 2~4 行：
   - 只留「數字 + 一句結論」，刪掉推導過程與客套鋪陳
   - 查詢網址、法規前提、共通試算假設一律移到 `sources` 欄（表格第 2 列），儲存格內不重複
   - `【現場確認】` 後面不要再解釋為什麼查不到，標記本身已表達
   - ⚠️ 每格最多一條，挑最重要的；其餘併入同一行
   - 同義並列的來源數字取 2 個（最高／最低）即可，不要列 4 個
2. 這樣仍超過 10,300 字元，才考慮合併分類（例如 1+2 併一份）換取份數不變
3. **最後才是拆 A／B**，且拆完總數仍不得超過 6 份
